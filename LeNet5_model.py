...
class LeNet_5(torch.nn.Module):
    ...
    def __init__(self, nof_classes):
        ...
        self.feat_extractor_part = torch.nn.Sequential(
            #Conv2D
            torch.nn.Conv2d(in_channels=1, out_channels=6, kernel_size=(5,5), padding=1, stride=1),
            torch.nn.ReLU(),
            #Sub-Sampling (average pooling)
            torch.nn.AvgPool2d(kernel_size=(2,2), stride=2, padding=0),
            #Conv2D
            torch.nn.Conv2d(in_channels=6, out_channels=16, kernel_size=(5,5), padding=0, stride=1),
            torch.nn.ReLU(),
            #Sub-Sampling (average pooling)
            torch.nn.AvgPool2d(kernel_size=(2,2), stride=2, padding=1),
            #Conv2D
            torch.nn.Conv2d(in_channels=16, out_channels=120, kernel_size=(5,5), padding=0),
            torch.nn.ReLU(),
        )
